...
def get_tics(pth):
    ...
    with open(pth, 'r') as f:
       for line in f:
           ...
           ticker = parts[1].lower().strip('"')
           tics.update({ticker: exchange})
    return tics
    ...
    if col_lst is not None:
        ...
        for column in col_lst:
            if column not in COLUMNS:
                raise Exception(f"col_lst contains a column {column} that is not found in COLUMNS")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        ...
        ticker_data = []
        for line in data:
            data_dic_ = line_to_dict(line)
            selected = {col: data_dic_[col] for col in col_lst}
            ticker_data.append(selected)
